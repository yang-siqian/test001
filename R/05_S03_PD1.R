...
exp1$Risk=-0.256*exp1$P2RX+0.127*exp1$PLAUR-0.163*exp1$PRKCZ+0.098*exp1$F12+0.083*exp1$SERPINE1+
...
# Fig5D_PD1
write.xlsx(survdata[,c(1,11,27,30,31)],file = 'files/Fig5.xlsx',sheetName = 'Fig5D_PD1',row.names = F,append = T)
png("figures/05D_PFSKM_risk_PRJEB23709_PD1.png",width = 1500,height = 1500,res=200)
...
# SupFig3A_PD1
write.xlsx(survdata[,c(1,27,30,26)],file = 'files/SupFig3.xlsx',sheetName = 'SupFig3A_PD1',row.names = F,append = T)
png("figures/S03A_PFSKM_risk_response_PD1.png",width = 1500,height = 1500,res=200)
p <- ggsurvplot(survfit(Surv(PFS, PFS.E) ~ Group, data=survdata),size=3,
                surv.median.line = "hv",
                title = "PRJEB23709_PD1",
...
write.xlsx(b,file = 'files/Fig5.xlsx',sheetName = 'Fig5C_PD1',row.names = F,append = T)
P <- ggplot(data=b,aes(x=group,y=value,fill=variable)) + theme_bw()+ylab('Percent')+xlab('')+ggtitle('PRJEB23709_PD1')+ylim(0,1)+
...
ggsave('figures/05C_barplot_PRJEB23709_PD1.png',P,width = 6,height = 6,dpi=200)

# SupFig3B_PD1
write.xlsx(survdata[,c(1,11,27,30)],file = 'files/SupFig3.xlsx',sheetName = 'SupFig3B_PD1',row.names = F,append = T)
library(survivalROC)
pd=survdata
...
png("figures/S03B_rocyear_risk_PD1.png",width = 1500,height = 1500,res=200)
...
# SupFig3C_CTLA4
write.xlsx(survdata[,c(1,11,26)],file = 'files/SupFig3.xlsx',sheetName = 'SupFig3C_PD1',row.names = F,append = T)
library(pROC)
roc1 <- roc(pd$Group,pd$Risk)
png("figures/S03C_roc_risk_PD1.png",width = 1500,height = 1500,res=200)
