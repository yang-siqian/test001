...
# SupFig2C_GSE31210
library(xlsx)
write.xlsx(clin,file = 'files/SupFig2.xlsx',sheetName = 'SupFig2_GSE31210',row.names = F)
library(survival)
library(survminer)
png("figures/S02C_OSKM_risk_GSE31210.png",width = 1500,height = 1500,res=200)
...
library(ggpubr)
#风险评分box图
# cb <- ggboxplot(clin,x = 'status',y = 'Risk',fill='status')+theme_bw()+ylab('Riskscore')+xlab('')+ggtitle('GSE31210')+ylim(-1,1)+
...
pd$Sample <- 1:nrow(pd)
sp <- ggplot(pd,aes(Sample,time,color=status))+geom_point()+geom_vline(aes(xintercept = 113),lty=2,lwd=1)+
  ...
  theme(axis.text.x = element_text(size=16,face='bold',color='black'),
        ...
        legend.title = element_blank(),legend.position = "none",
        panel.grid = element_blank(),panel.border = element_rect(size=1.5))
rp <- ggplot(pd,aes(Sample,Risk,color=Group))+geom_point()+geom_vline(aes(xintercept = 113),lty=2,lwd=1)+
  theme_bw()+scale_colour_manual(values = c('red','blue'))+ylab('Riskscore')+xlab('')+ggtitle('GSE31210')+
  theme(plot.title = element_text(hjust = 0.5,vjust = 1.5,face = 'bold',size = 25),axis.text.x = element_text(size=18,face='bold',color='black'),
        ...
        panel.grid = element_blank(),panel.border = element_rect(size=1.5))
ggsave('figures/S02A_survpoint_GSE31210.png',sp,width = 5,height =3,dpi=1000)
ggsave('figures/S02A_riskpoint_GSE31210.png',rp,width = 5,height = 3,dpi=1000)
...
save_pheatmap_pdf(p, "figures/S02A_pheat_GSE31210.pdf")
...
png("figures/S02B_rocyear_risk_GSE31210.png",width = 1500,height = 1500,res=200)
plot(troc.1$FP, troc.1$TP, type="l",col="red", xlim=c(0,1), ylim=c(0,1), lwd=4,cex.axis=1.5,ann = F)
title(main='GSE31210',xlab="1-Specificity", ylab="Sensitivity",cex.main = 2.5, font.main= 2,cex.lab=1.6,font.lab=2)
...
write.xlsx(pd[,c(1,17:19,22:30)],file = 'files/Fig2.xlsx',sheetName = 'Fig2G_GSE31210',row.names = F,append = T)
...
png("figures/02G_rocmethods_bsr_GSE31210.png",width = 1500,height = 1500,res=200)
plot(troc.1$FP, troc.1$TP, type="l",col="red", xlim=c(0,1), ylim=c(0,1), lwd=4,cex.axis=1.5,ann = F)
title(main='GSE31210',xlab="1-Specificity", ylab="Sensitivity",cex.main = 2.5, font.main= 2,cex.lab=1.6,font.lab=2)
abline(0,1,col='gray',lty=2,lwd=3)
